...
const process_cli = function(argv_vals){
  ...
  const process_url = async function(type, url){
        ...
        case 2:
          {
            ...
            let ffmpegcmd = `ffmpeg -allowed_extensions ALL -i "${path.join(outputdir, 'hls', 'master.m3u8')}" -c copy -movflags +faststart "${path.join(outputdir, 'mp4', 'video.mp4')}"`
  ...
  return process_url(argv_vals["--url-type"], argv_vals["--url"])
}
