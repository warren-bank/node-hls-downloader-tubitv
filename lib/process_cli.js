...
const process_cli = function(argv_vals){
  ...
  const process_video_data = function(data, type, url){
    ...
    if (!argv_vals["--quiet"]) {
      ...
      let ffmpegcmd = `cd "${configHLS["--directory-prefix"]}" && mkdir "${path.join('..', 'mp4')}" & ffmpeg -allowed_extensions ALL -i "master.m3u8" -c copy -movflags +faststart ${configHLS["--mp4-ffmpeg-options"] || ''} "${path.join('..', 'mp4', 'video.mp4')}"`
